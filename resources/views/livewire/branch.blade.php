    ...
    <div class="py-12 rounded-md">
        <div class="w-4/6 mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-300 overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
                ...
                @if($isModalOpen)
                @include('livewire.create-branch')
                @endif
                <table class="table-fixed rounded w-full">
                    <thead class="text-center">
                        <tr class=" bg-gray-400">
                            ...
                            <th class="px-4 py-2">Branch Address</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="border px-4 py-2">{{ $tree->id }}</td>
                            <td class="border px-4 py-2">{{ $tree->branch_name }}</td>
                            <td class="border px-4 py-2">{{ $tree->branch_address}}</td>
                            <td class="border px-4 py-2">
                                <button wire:click="edit({{ $tree->id }})"
                                    class="flex px-4 py-2 bg-blue-600 hover:bg-blue-500 text-gray-900 cursor-pointer rounded">Update</button>
                                <button wire:click="delete({{ $tree->id }})"
