        ...
        <div class="w-11/12 mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-300 overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
                ...
                <button wire:click="create()"
                    class="ml-80 my-4 inline-flex justify-center w-1/2 rounded-md border border-transparent px-4 py-2 bg-gray-700 text-base font-bold text-white shadow-sm hover:bg-red-700">
                    Create A Member
                </button>
                @if($isModalOpen)
                ...
                <table class="table-fixed rounded-lg w-full">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="border px-4 py-2">{{ $mouse->id }}</td>
                            <td class="border px-4 py-2">{{ $mouse->name_f }}</td>
                            <td class="border px-4 py-2">{{ $mouse->trn}}</td>
                            <td class="border px-4 py-2">{{ $mouse->el_mail}}</td>
                            <td class="border px-4 py-2">{{ $mouse->numb}}</td>
                            <td class="border px-4 py-2">{{ $mouse->add}}</td>
                            <td class="border px-4 py-2">
                                <button wire:click="edit({{ $mouse->id }})"
                                    class="flex px-4 py-2 bg-blue-600 hover:bg-blue-500 text-gray-900 cursor-pointer rounded">Update</button>
                                <button wire:click="delete({{ $mouse->id }})"
